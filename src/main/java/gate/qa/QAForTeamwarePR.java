 ...
 * created for annotating annotators. This makes it impossible to run QA on such
 * documents as same name annotation sets in different documents may refer to
 * the annoations created by different annotators. This is especially the case
 * when a requirement is to compute IAA between individual annotators. This PR
 * generates a summary of agreements among annotators. It does this by pairing
 * individual annotators. It also compares each individual annotator's
 * annotations with those available in the consensus annotation set in the
 * respective documents.
 * 
 * @author niraj
 */
@CreoleResource(name = "QA Summariser for Teamware", comment = "The Quality Assurance PR for teamware", helpURL = "http://gate.ac.uk/userguide/sec:eval:qaForTW")
public class QAForTeamwarePR extends AbstractLanguageAnalyser implements
	...
	/**
	 * used for keeping record of documents annotated by individual annotators
	 */
	...
	/**
	 * If this parameter is provided, it is assumed that the annonymous mode is
	 * not in place and therefore the names of annotation sets should be
	 * considered as author names.
	 */
	private List<String> annotationSetNamesAsAnnotators = null;
	...
	private static final String ANNOTATOR_PREFIX = "TW_Annotator_";
	...
	public Resource init() throws ResourceInstantiationException {
		...
		assurancePR = (QualityAssurancePR) Factory.createResource(
				"gate.qa.QualityAssurancePR", Factory.newFeatureMap(),
				hideParams);
		...
		controller = (SerialAnalyserController) Factory.createResource(
				...
				Factory.newFeatureMap(), hideParams);
	...
	public void execute() throws ExecutionException {
		...
		// this is where we store names of annotators
		Set<String> annotators = new HashSet<String>();

		// we ignore the following block if anonymous mode was disabled
		// and we determine that by checking if user has provided values
		// for annotationSetNamesAsAnnotators
		if (annotationSetNamesAsAnnotators == null
				|| annotationSetNamesAsAnnotators.isEmpty()) {
			...
			for (Object featureName : document.getFeatures().keySet()) {
				if (featureName instanceof String) {
					...
					if (fName.startsWith("safe.asname.")) {
						String annotatorName = (String) document.getFeatures()
								.get(fName);
			...
			// we need to ignore some of the annotators if we are instructed to
			// do
			// so
			...
			// find out which of the annotationSetNamesAsAnnotators
			// has annotated this document
			for (String annotatorName : annotationSetNamesAsAnnotators) {
				if (document.getNamedAnnotationSets()
						.containsKey(annotatorName)) {
					annotators.add(annotatorName);
				}
			}
		}
		...
		for (String annName : annotatorToDocuments.keySet()) {
			...
			for (Object aDoc : docs) {
				...
				createAnnSet(gateDoc, getSetName(gateDoc, annName), ANNOTATOR_PREFIX + annName);
			...
			if (consensusExists) {
				calculateIAA(corpus, ANNOTATOR_PREFIX + annName, CONSENSUS_AS_NAME, annName);
			}
			...
			for (int k = corpus.size() - 1; k >= 0; k--) {
				...
				aDoc.removeAnnotationSet(ANNOTATOR_PREFIX + annName);
			}
		...
		for (String annotatorsPair : annotatorsPairToDocuments.keySet()) {
			...
			String annotator1 = annotatorsPair.substring(0,
					annotatorsPair.indexOf(';'));
			String annotator2 = annotatorsPair.substring(annotatorsPair
					.indexOf(';') + 1);
			// one doc at a time
			for (Object aDoc : docs) {
				...
				createAnnSet(gateDoc, getSetName(gateDoc, annotator1),
						ANNOTATOR_PREFIX + annotator1);
				createAnnSet(gateDoc, getSetName(gateDoc, annotator2),
						ANNOTATOR_PREFIX + annotator2);
				// adding doc to the corpus
				corpus.add(gateDoc);
			}
			// calculate IAA
			calculateIAA(corpus, ANNOTATOR_PREFIX + annotator1,
					ANNOTATOR_PREFIX + annotator2, annotator1 + "-"
							+ annotator2);
			...
			for (int k = corpus.size() - 1; k >= 0; k--) {
				...
				aDoc.removeAnnotationSet(ANNOTATOR_PREFIX + annotator1);
				aDoc.removeAnnotationSet(ANNOTATOR_PREFIX + annotator2);
			}
			// delete the corpus
			Factory.deleteResource(corpus);
		}
		...
		for (File file : resultFiles) {
			...
			List<Annotation> cols = new ArrayList<Annotation>(omSet
					.getContained(Utils.start(row), Utils.end(row)).get("td"));
			Collections.sort(cols, new OffsetComparator());
			// only interested in the last column
			r.micro = Double.parseDouble(Utils.stringFor(aDoc,
					cols.get(cols.size() - 1)));
			// consider second last row for macro summary
			row = rows.get(rows.size() - 2);
			cols = new ArrayList<Annotation>(omSet.getContained(
					Utils.start(row), Utils.end(row)).get("td"));
			Collections.sort(cols, new OffsetComparator());
			// only interested in the last column
			r.macro = Double.parseDouble(Utils.stringFor(aDoc,
					cols.get(cols.size() - 1)));
		...
		int totalAuthors = consensusExists ? columnAuthorNames.size() - 1
				: columnAuthorNames.size();
		buffer.append("<br><b>Avg. IAA macro avg:</b> "
				+ (f.format((double) annotatorMacro / totalAuthors)));
		buffer.append("<br><b>Avg. IAA micro avg:</b> "
				+ (f.format((double) annotatorMicro / totalAuthors)));
		buffer.append("</body>\n</html>");
	...
	/**
	 * A method that adds two columns to the buffer - one for the macro figure
	 * and the other one for the micro figure. It also adds relevant style tag
	 * to give a proper color to each cell depending on the value of macro and
	 * micro figures. See documentation of the getStyleTag(double) for more
	 * information on how cell backgrounds are color coded.
	 * 
	 * @param buffer
	 * @param macro
	 * @param micro
	...
	/**
	 * Gets the style tag based on the score. The color green is used for a cell
	 * background to indicate full agreement (i.e. 1.0). The background color
	 * becomes lighter as the agreement reduces towards 0.5. At 0.5 agreement,
	 * the background color of a cell is fully white. From 0.5 downwards, the
	 * color red is used and as the agreement reduces further, the color becomes
	...
	private String getStyleTag(double score) {
		...
		return "style=\"background-color: rgba(" + c.getRed() + ","
				+ c.getGreen() + "," + c.getBlue() + ","
				+ ((double) c.getAlpha() / 100) + ")\"";
	}

	/**
	 * Given the annotator name, the method finds out the annotation set which
	 * contains annotations for that annotator
	 * 
	 * @param doc
	 * @param annotatorName
	 * @return
	...
	private String getSetName(Document doc, String annotatorName) {
		...
		// if the annotatorName is inside the annotationSetNamesToAnnotators
		// it means, this is not an anonymous mode
		if (annotationSetNamesAsAnnotators != null
				&& annotationSetNamesAsAnnotators.contains(annotatorName)) {
			return annotatorName;
		}

		// otherwise find out the annotator name from the document features
		String inputAS = null;
	...
	private void createAnnSet(Document doc, String inputAS, String outputAS) {
		// if both inputAS and outputAS are same, we just return
		// no need to create a temporarity annotation set
		if (inputAS.equals(outputAS))
			return;
		...
		for (Annotation a : inAS) {
			outAS.add(a.getStartNode(), a.getEndNode(), a.getType(),
					a.getFeatures());
		}
	}
	...
	private void calculateIAA(Corpus corpus, String keyAS, String responseAS,
		...
		assurancePR
				.setFeatureNames(featureNames == null ? new ArrayList<String>()
						: featureNames);
		...
		File corpusFile = new File(outputFolderUrl.getFile(),
				"corpus-stats.html");
		corpusFile.delete();
	 ...
	 * 
	 * @return the annotation types for which the states should be calculated
	 */
	 ...
	 * Annotation types for which the stats should be calculated
	 * 
	 * @param annotationTypes the annotation types for which stats should be calculated
	 */
	@RunTime
	@CreoleParameter
	 ...
	 * 
	 * @return the feature names for which the states should be calculated
	 */
	 ...
	 * Features names for which the stats should be calculated
	 * 
	 * @param featureNames the feature names for which the stats should be calculayed
	 */
	@RunTime
	@Optional
	@CreoleParameter
	 ...
	 * 
	 * @return the measure used for stats calculation
	 */
	 ...
	 * Measure to use for stats calculation
	 * 
	 * @param measure the measure to use for the stats calculation
	 */
	@RunTime
	@CreoleParameter
	 ...
	 * 
	 * @return the URL of the folder in which to store the output files
	 */
	 ...
	 * URL of the folder to store output files into
	 * 
	 * @param outputFolderUrl the URL of the folder in which to store the output files
	 */
	@RunTime
	 ...
	 * 
	 * @return the list of annotators to ignore when cslculating IAA
	 */
	public List<String> getAnnotatorsToIgnore() {
		return annotatorsToIgnore;
	}

	/**
	 * annotators to ingore when computing IAA. Please note that if there's at
	 * least one value provided for the annotationSetNamesAsAnnotators, this
	 * parameter will have no effect.
	 * 
	 * @param annotatorsToIgnore the list of annotators to ignore when doing the IAA calculation
	 */
	@RunTime
	@Optional
	@CreoleParameter
	...
	public List<String> getAnnotationSetNamesAsAnnotators() {
		return annotationSetNamesAsAnnotators;
	}

	/**
	 * If this parameter is provided, it is assumed that the anonymous mode is
	 * not in place and therefore the names of annotation sets should be
	 * considered as author names. Also note that if this parameter is provided
	 * the parameter annotatorsToIgnore is ignored entirely. In other words, it
	 * has no effect.
	 * 
	 * @param annotationSetNamesAsAnnotators
	 */
	@RunTime
	@Optional
	@CreoleParameter
	public void setAnnotationSetNamesAsAnnotators(
			List<String> annotationSetNamesAsAnnotators) {
		this.annotationSetNamesAsAnnotators = annotationSetNamesAsAnnotators;
	}
